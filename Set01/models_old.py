    ...
    for word in querywrds:
        ...
        for docid in docids:
            try:
                ...
                crp_freq = dcfreq[word]['crp_freq']
            except KeyError:
                #print word + " -> is absent"
                continue
            bckgrd = float(crp_freq - term_freq) / (crp_len - doc_len)
            fregrd = float(term_freq) / doc_len
            ratio = fregrd / bckgrd
            # Calulate lamba
            if ratio > 1:
                lmbda = .7
            elif ratio < 1 and ratio > .1:
                lmbda = .55
            else:
                lmbda = .3
...
def main():
    ...
    modelgen(elasrch, querymap, tfreq, dcfreq, doclen, "jelinekmecer", "1_JLKMCRlaplace.txt")
