...
def docfreq(catdict, docid, term):
    ifile = open("Index.txt", 'r')
    ifile.seek(catdict[term][0])
    # Read the inverted in bytes i.e. stopoffset - startoffset
    invlist = ifile.read(catdict[term][1] - catdict[term][0])
...
def proximsrch(catdict, query, statdict, dlendict):
    ...
    for itr in xrange(0, len(querywrds) - 1):
        ...
        # Get term pos for cur and next word
        try:
            ...
            curid = list()
            curposdict = {}
            # Get all docid in curid and corresponding term pose in curpos dict
        ...
        # Get common doc for both terms in Set
            ...
            for docid in commomdoc:
                ...
                for dumyitr in xrange(0, (len(cposlst) + len(nposlst) - 1)):
                    ...
                    if citr >= (len(cposlst) - 1):
                        citr = (len(cposlst) - 1)
                # print str(citr) + " " + str(nitr)
                # print row
                score = (1500 - row) * (len(cposlst) + len(nposlst)) / (dlendict[docid]) + vocabsize
                # deno_score = dlendict[docid] + vocabsize
                # score = float(num_score) / deno_score
                if docid not in doctf:
                    doctf[docid] = score
                else:
...
def main():
    ...
    # f = search(catdict, stem("fibers"))
    # i = search(catdict, stem("insulation"))
    # p = search(catdict, stem("products"))
    # t = Set(i) & Set(p)
    #print len(t)
    ...
    modelgen(catdict, querymap, dlendict, diddict, statdict, "proximity", "ProximitySearch2.txt")
